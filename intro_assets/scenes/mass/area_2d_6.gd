...
@onready var color_picker = $bigguy/CanvasLayer2/Control/ColorPicker # ColorPicker node in the UI
@onready var sprite_to_color = $bigguy # The sprite node you want to change color
@onready var reset_button = $bigguy/CanvasLayer2/Control/reset # Reset button to reset color
...
func _on_click(viewport, event, shape_idx):
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
		...
		if clicked_shape.name == "bigguy2":
			print("game clicked")
			_on_reset_pressed()

		# If the "gamee" shape is clicked, show the color picker
		if clicked_shape.name == "bigguy":
			print("gamee clicked")
