...
@onready var text_label = $CanvasLayer2/Control2/RichTextLabel
@onready var robodim = $CanvasLayer2/Control/Robodim/AnimatedSprite2D
@onready var note = $BaseMass/CanvasLayer/Area2D/note2
...
func _ready():
	note.visible = true
	dialogue_data = load("res://intro_assets/scenes/mass/note_scne_DIALOGUE.gd").new().get_dialogue()
	robodim.play()
