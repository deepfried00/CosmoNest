...
@onready var repaired_lss = $RepairedLss  # Adjust this path to match the actual scene tree
...
func some_trigger():
	if repaired_lss:
		repaired_lss.show_info_popup_text()

func _input(event):
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
		if dialogue_finished:
			 # Stop music when final click after dialogue
			return
